...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    # 创建类别映射文件
    if "classes" in interactive_state and interactive_state["classes"]:
    ...
    # 创建mask到类别的映射
    mask_to_class = {}
    class_id_map = {}  # 映射原始class_id到连续编号(从0开始)
    if "classes" in interactive_state:
        for idx, (class_id, class_info) in enumerate(sorted(interactive_state["classes"].items())):
            class_id_map[class_id] = idx  # 映射到0开始的连续编号
            for mask_id in class_info["mask_ids"]:
                mask_to_class[mask_id] = (class_id, class_info["name"])
    ...
    for frame_idx, mask in enumerate(video_state["masks"]):
        ...
        if frame_idx < video_state["select_frame_number"]:
            original_frame = video_state["origin_images"][frame_idx]
        else:
            original_frame = following_frames[frame_idx - video_state["select_frame_number"]]
        ...
        cv2.imwrite(os.path.join(yolo_dir, "images", img_filename), 
                   cv2.cvtColor(original_frame, cv2.COLOR_RGB2BGR))
        ...
        for label in unique_labels:
            ...
            x1, y1, x2, y2 = bbox
            x_center = ((x1 + x2) / 2) * scale_ratio / original_w
            y_center = ((y1 + y2) / 2) * scale_ratio / original_h
            width = (x2 - x1) * scale_ratio / original_w
            height = (y2 - y1) * scale_ratio / original_h
            
            # 获取类别信息
            mask_id = label - 1  # mask编号从1开始，调整为0开始
            class_info = mask_to_class.get(mask_id, (0, "unknown"))
            mapped_class_id = class_id_map.get(class_info[0], 0)  # 映射到0开始的连续编号
            
            # 保存到labels文件
            label_lines.append(f"{mapped_class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")
            ...
            frame_bboxes.append((bbox_original, class_info[1]))  # (bbox, class_name)
        
        # 写入标签文件
        label_filename = f"frame_{frame_idx:04d}.txt"
        with open(os.path.join(yolo_dir, "labels", label_filename), "w") as f:
    ...
    for idx, frame in enumerate(video_state["origin_images"]):
        ...
        if idx < video_state["select_frame_number"]:
            ...
            if frame_idx < len(bboxes_list):
                painted_frame = original_frame.copy()
                for bbox, class_name in bboxes_list[frame_idx]:
                    # 绘制边界框
                    color = (0, 255, 0)  # 绿色
                    cv2.rectangle(painted_frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), color, 2)
                    ...
                    # 标签背景
                    cv2.rectangle(painted_frame, 
                                 (bbox[0], bbox[1] - label_height - 5),
                                 (bbox[0] + label_width, bbox[1] - 5),
                                 color, -1)
    ...
    operation_log.append(("", f"YOLO格式数据已保存到: {yolo_dir}"))
